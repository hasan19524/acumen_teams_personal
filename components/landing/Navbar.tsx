...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <span className="text-slate-900 font-bold text-lg leading-tight tracking-tight hidden sm:block whitespace-nowrap">
                Acumen{" "}
                <span className="font-semibold text-slate-500">Teams</span>
              </span>
            </Link>
      ...
      {/* Mobile Menu — rendered OUTSIDE <nav> on purpose. <nav> gets backdrop-blur
        (a filter) when open, and any filter on an ancestor turns it into the
        containing block for position:fixed descendants, which was collapsing
        this panel's fixed positioning. Keeping it as a sibling of <nav> means
        it stays fixed to the real viewport. */}
      <div
        className={`md:hidden fixed inset-x-0 top-[64px] bottom-0 z-40 bg-white transition-all duration-300 ease-out ${
          mobileOpen
            ? "opacity-100 translate-y-0 pointer-events-auto"
            : "opacity-0 -translate-y-2 pointer-events-none"
        }`}
      >
        <div className="overflow-y-auto flex flex-col px-4 pt-2 pb-8 max-h-[calc(100vh-64px)]">
